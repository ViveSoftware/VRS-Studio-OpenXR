...
namespace Wave.Essence.Sample.Hand
{
    ...
    public class HandController : MonoBehaviour
    {
        ...
        HandData handData;
        InputDevice inputDeviceHand;
        InputDevice inputDeviceRightController;
        InputDevice inputDeviceLeftController;
        ...
        public class JointData
        {
            ...
            public XrHandJointEXT joint = XrHandJointEXT.XR_HAND_JOINT_WRIST_EXT;
            ...
            public void SetJointData(int jointId)
            {
                ...
                this.joint = (XrHandJointEXT)jointId;
                this.transform = null;
        ...
        Transform GetCameraRig()
        {
            ...
            if (cameraRig == null && VIVERig.Instance != null)
                cameraRig = VIVERig.Instance.transform;
        ...
        {
            if (!inputDeviceHand.isValid || !inputDeviceLeftController.isValid || !inputDeviceRightController.isValid)
            {
            ...
            handData = CachedHand.Get(isLeft);
        }

        bool IsCtrlsTracked()
        {
            bool isTracked = false;
            if (inputDeviceRightController.isValid)
                inputDeviceRightController.TryGetFeatureValue(CommonUsages.isTracked, out isTracked);
            if (isTracked) return true;
            if (!isTracked && inputDeviceLeftController.isValid)
                inputDeviceLeftController.TryGetFeatureValue(CommonUsages.isTracked, out isTracked);
            return isTracked;
        }

        bool IsTracked()
        {
            if (!inputDeviceHand.isValid) return false;
            if (inputDeviceHand.TryGetFeatureValue(CommonUsages.isTracked, out bool isTracked))
                return isTracked && handData.isTracked;
            return false;
        }
        ...
        private void UpdateValidPoses()
        {
            ...
            for (int i = 0; i < jointDataList.Length; i++)
            {
                ...
                if (hasBodyTracking && jd.joint == XrHandJointEXT.XR_HAND_JOINT_WRIST_EXT)
                    continue;
                ...
                {
                    if (applyAllPosition || jd.joint == XrHandJointEXT.XR_HAND_JOINT_WRIST_EXT)
                    {
        ...
        public void UpdatePoses()
        {
            ...
            CheckInputDevice();
            bool isValid = inputDeviceHand.isValid;
            ...
            if (isTracked)
            {
                ...
                // Wrist
                jointDataList[1].hasPose |= handData.GetJointPosition(JointType.Wrist, ref jointDataList[1].positionT);
                jointDataList[1].hasPose |= handData.GetJointRotation(JointType.Wrist, ref jointDataList[1].rotationT);

                // Thumb
                jointDataList[2].hasPose |= handData.GetJointPosition(JointType.Thumb_Joint0, ref jointDataList[2].positionT);
                jointDataList[2].hasPose |= handData.GetJointRotation(JointType.Thumb_Joint0, ref jointDataList[2].rotationT);
                jointDataList[3].hasPose |= handData.GetJointPosition(JointType.Thumb_Joint1, ref jointDataList[3].positionT);
                jointDataList[3].hasPose |= handData.GetJointRotation(JointType.Thumb_Joint1, ref jointDataList[3].rotationT);
                jointDataList[4].hasPose |= handData.GetJointPosition(JointType.Thumb_Joint2, ref jointDataList[4].positionT);
                jointDataList[4].hasPose |= handData.GetJointRotation(JointType.Thumb_Joint2, ref jointDataList[4].rotationT);

                // Index
                jointDataList[6].hasPose |= handData.GetJointPosition(JointType.Index_Joint0, ref jointDataList[6].positionT);
                jointDataList[6].hasPose |= handData.GetJointRotation(JointType.Index_Joint0, ref jointDataList[6].rotationT);
                jointDataList[7].hasPose |= handData.GetJointPosition(JointType.Index_Joint1, ref jointDataList[7].positionT);
                jointDataList[7].hasPose |= handData.GetJointRotation(JointType.Index_Joint1, ref jointDataList[7].rotationT);
                jointDataList[8].hasPose |= handData.GetJointPosition(JointType.Index_Joint2, ref jointDataList[8].positionT);
                jointDataList[8].hasPose |= handData.GetJointRotation(JointType.Index_Joint2, ref jointDataList[8].rotationT);
                jointDataList[9].hasPose |= handData.GetJointPosition(JointType.Index_Joint3, ref jointDataList[9].positionT);
                jointDataList[9].hasPose |= handData.GetJointRotation(JointType.Index_Joint3, ref jointDataList[9].rotationT);

                // Middle
                jointDataList[11].hasPose |= handData.GetJointPosition(JointType.Middle_Joint0, ref jointDataList[11].positionT);
                jointDataList[11].hasPose |= handData.GetJointRotation(JointType.Middle_Joint0, ref jointDataList[11].rotationT);
                jointDataList[12].hasPose |= handData.GetJointPosition(JointType.Middle_Joint1, ref jointDataList[12].positionT);
                jointDataList[12].hasPose |= handData.GetJointRotation(JointType.Middle_Joint1, ref jointDataList[12].rotationT);
                jointDataList[13].hasPose |= handData.GetJointPosition(JointType.Middle_Joint2, ref jointDataList[13].positionT);
                jointDataList[13].hasPose |= handData.GetJointRotation(JointType.Middle_Joint2, ref jointDataList[13].rotationT);
                jointDataList[14].hasPose |= handData.GetJointPosition(JointType.Middle_Joint3, ref jointDataList[14].positionT);
                jointDataList[14].hasPose |= handData.GetJointRotation(JointType.Middle_Joint3, ref jointDataList[14].rotationT);

                // Ring
                jointDataList[16].hasPose |= handData.GetJointPosition(JointType.Ring_Joint0, ref jointDataList[16].positionT);
                jointDataList[16].hasPose |= handData.GetJointRotation(JointType.Ring_Joint0, ref jointDataList[16].rotationT);
                jointDataList[17].hasPose |= handData.GetJointPosition(JointType.Ring_Joint1, ref jointDataList[17].positionT);
                jointDataList[17].hasPose |= handData.GetJointRotation(JointType.Ring_Joint1, ref jointDataList[17].rotationT);
                jointDataList[18].hasPose |= handData.GetJointPosition(JointType.Ring_Joint2, ref jointDataList[18].positionT);
                jointDataList[18].hasPose |= handData.GetJointRotation(JointType.Ring_Joint2, ref jointDataList[18].rotationT);
                jointDataList[19].hasPose |= handData.GetJointPosition(JointType.Ring_Joint3, ref jointDataList[19].positionT);
                jointDataList[19].hasPose |= handData.GetJointRotation(JointType.Ring_Joint3, ref jointDataList[19].rotationT);

                // Pinky
                jointDataList[21].hasPose |= handData.GetJointPosition(JointType.Pinky_Joint0, ref jointDataList[21].positionT);
                jointDataList[21].hasPose |= handData.GetJointRotation(JointType.Pinky_Joint0, ref jointDataList[21].rotationT);
                jointDataList[22].hasPose |= handData.GetJointPosition(JointType.Pinky_Joint1, ref jointDataList[22].positionT);
                jointDataList[22].hasPose |= handData.GetJointRotation(JointType.Pinky_Joint1, ref jointDataList[22].rotationT);
                jointDataList[23].hasPose |= handData.GetJointPosition(JointType.Pinky_Joint2, ref jointDataList[23].positionT);
                jointDataList[23].hasPose |= handData.GetJointRotation(JointType.Pinky_Joint2, ref jointDataList[23].rotationT);
                jointDataList[24].hasPose |= handData.GetJointPosition(JointType.Pinky_Joint3, ref jointDataList[24].positionT);
                jointDataList[24].hasPose |= handData.GetJointRotation(JointType.Pinky_Joint3, ref jointDataList[24].rotationT);
            }
        ...
        private void Update()
        {
            if (!doUpdate || (VRSBodyTrackingManager.Instance && !VRSBodyTrackingManager.Instance.IsTracking()))
                return;
            UpdatePoses();
        }
